...
@app.command()
def show(config_path: Path = Paths().config_path):
    settings = EventsSettings.load(config_path)
    run(show_main, settings, Console())

@app.command()
def info(config_path: Path = Paths().config_path):
    rich.print("")
...
@app.command()
def mkconfig(config_path: Path = Paths().config_path, force: bool = False):
    rich.print()
    if config_path.exists() and not force:
        rich.print(f":warning-emoji:    [orange_red1]Config path [/]{config_path} [orange_red1]already exists.")
        ...
        if config_path.exists():
            rich.print(f":warning-emoji:    [orange_red1][bold]Overwritting config file [/][/]{config_path}.")
        else:
